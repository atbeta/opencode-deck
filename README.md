...
巡检间隔内若 session **idle** 且任务未完成，Harness 会发送进度确认 prompt（要求 Agent 回复 `STEP DONE` / `TASK COMPLETE` 或继续执行）。

## 权限与 Harness（重要）

**OpenDeck 不会自动批准 OpenCode 的 tool 权限。** Harness 无人值守运行时，若 Agent 触发 `read` / `edit` / `bash` 等需要审批的操作，而 OpenCode 规则为 `ask`，任务会卡在 **waiting**，check log 显示 *Waiting for permission or input*，直到有人在 **OpenCode Desktop / TUI** 里手动点允许。

这与「Agent 问 should I proceed?」不同：后者 Harness 可用 prompt 自动回复；**tool permission 必须在 OpenCode 侧预先放开或人工批准**。

### OpenDeck 侧（工作区白名单）

OpenDeck 只限制 **Dispatch / Harness 能选哪个目录**，不控制 OpenCode 工具权限：

| 变量 | 作用 |
|------|------|
| `OPENDECK_ALLOWED_ROOTS` | 允许派发的 workspace 根路径（冒号分隔） |
| `OPENDECK_STRICT_ROOTS=true` | 强制只能使用上述目录 |

示例（`.env`）：

```bash
OPENDECK_ALLOWED_ROOTS=/Users/you/Projects
OPENDECK_STRICT_ROOTS=true
```

### OpenCode 侧（Agent 权限规则）

权限由 **OpenCode 配置文件**（可通过 `GET /config` 查看当前生效配置）中的 `agent.<name>.permission` 决定。Harness 创建的 session 默认使用 spec 里的 `agent`（未指定时为 `opencode`，通常落到 `build` 等默认 agent）。

常见 `action` 取值：

| 值 | 含义 | 对 Harness 的影响 |
|----|------|-------------------|
| `allow` | 自动允许 | 不卡住，适合无人值守 |
| `ask` | 每次询问 | **会卡住**，需人工批准 |
| `deny` | 拒绝 | Agent 无法执行该操作 |

若 `build` agent 的 `bash` 为 `"*": "ask"`（很多默认配置如此），Harness 跑实现类任务时几乎一定会等权限。

**推荐做法（仅对可信的本机 workspace）：** 在 OpenCode 配置中为 Harness 使用的 agent 放宽规则，例如：
...
若 Harness 目标目录固定、且你完全信任该环境，可对**该 agent** 将 `read` / `edit` / `bash` 的 `"*"` 设为 `"allow"`（风险自负：Agent 可在该 workspace 内任意执行 shell 与改文件）。

修改 OpenCode 配置后需**重启 `opencode serve`**（或按你当前 OpenCode 版本的配置热加载方式）使规则生效。

### Harness spec 中的 agent

```yaml
name: Fix SSE reconnect
workspace: /Users/you/Projects/my-app
agent: opencode   # 或 build / plan 等，权限跟随该 agent 的 permission 块
mode: normal
```

确保 spec 里的 `agent` 与你在 OpenCode 里放宽权限的 agent 名称一致。

### 如何确认是否卡在权限

1. **OpenDeck**：Harness 任务状态为 `waiting`，summary 含 permission / wait。
2. **OpenCode Desktop**：打开对应 session，查看是否有待批准的 tool 请求。
3. **API**（调试用）：

```bash
curl -s -u "$OPENCODE_SERVER_USERNAME:$OPENCODE_SERVER_PASSWORD" \
  "$OPENCODE_SERVER_URL/permission"
```

返回非空数组表示有全局 pending 权限；也可用 `GET /session/{sessionID}/permission/request`。

### 小结

| 场景 | 建议 |
|------|------|
| 本机可信项目、希望 Harness 无人值守 | OpenCode agent 对 `read`/`edit`/`bash` 使用 `allow` 或细化 allow 规则 |
| 需要人工把关每次写文件/跑命令 | 保持 `ask`，在 Desktop 里随 Harness session 手动批准 |
